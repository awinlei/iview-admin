...
<template>
    <div class="echart_tab_style">
      ...
      <!-- <span>渲染出动态的tabs {{cardId}}</span> -->
        <Tabs type="card" :animated="true" @on-click="handleTabChange">
            <TabPane v-for="(tab,index) in tabs.tabList" :key="tab" :label="tab"><div style="width:100%;height:100%;" v-bind:id="tabs.cardId + index"></div></TabPane>
        </Tabs>
    </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tabId: 0, //默认的页签选择
      queryParams: {
        cmdId: "getDataRequest", //api查询接口制定
        type: 0, //api查询接口参数制定
  ...
  methods: {
    ...
    showChart() {
      ...
      let chartId = this.cardId + this.tabId;
      // 设定图表的样式主题
    ...
    handleTabChange(name) {
      ...
      // 组装参数
      this.queryParams.cmdId = this.cardId;
      this.queryParams.type = this.tabId;
      // 获取数据并渲染
      this.mockChartData();
    }
  },
